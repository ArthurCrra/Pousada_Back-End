...
def consultar_pagamentos():
    ...
    try:
        ...
        print(f"Pagamentos fetched: {pagamentos}")  # Log the fetched payments
        ...
        print(f"Error fetching payments: {error}")  # Log the error details
        return jsonify({"status": "error", "message": f"Erro ao consultar pagamentos: {error}"}), 500
    finally:
        cursor.close()
        conn.close()
